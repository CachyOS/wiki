...
They're many ways to gather logs from your system. Here is a couple of examples and tools you can use:
...
- X program is no longer starting:
  - They are many reasons why a graphical program might not start. The best way to gather logs for this kind of issue is to run the program from a terminal. This way you can see any error messages or output that might help diagnose the problem.
  - Example:
    ```sh
    firefox
    ```
...
<details>
<summary>Following logs in Real Time:</summary>
```bash
journalctl -f
```
:::tip
...
<details>
<summary>Bluetooth related issues:</summary>
```bash
...
<details>
<summary>WiFi or network adapter issues:</summary>
```bash
...
#### Why are there two identical wine packages in some repositories?

- `cachyos-v3/v4/wine` = Wine with NTSync patchset
- `cachyos-extra-v3/v4/znver4/wine` = Vanilla Wine **rebuilt by CachyOS**

Whenever Wine merges the NTSync patchset, we will remove the `cachyos-v3/v4/wine` package. The `cachyos-extra-v3/v4/znver4/wine` package will continue to be available.
