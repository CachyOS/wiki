...
5. Determines and checks if the system type is either **UEFI or BIOS/MBR** > prompt the user to select a bootloader depending on the type.
...
Sometimes this could happen due to a desynchronization of a mirror and by waiting a bit they tend to fix themselves or simply your system has broken keyrings.
But if even after waiting you are getting the same error. Try one of the two possible workarounds:

<Tabs>

<TabItem label='First workaround'>
...
<TabItem label='Second workaround'>

- Open CachyOS Hello and go to **Apps/Tweaks** and click on the **Reset keyrings** button.
...
As the error suggests, pacman is trying to update or install a package that contains the same files as those already present in your system. You can resolve this issue by removing the conflicting files manually. For more information, refer to the [Arch Wiki](https://wiki.archlinux.org/title/Pacman#%22Failed_to_commit_transaction_(conflicting_files)%22_error)
