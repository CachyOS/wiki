...
1. Download the [file](https://mirror.cachyos.org/ISO/desktop/260124/cachyos-desktop-linux-260124.iso.sha256) containing the SHA256 hash **(Open it with a text editor, e.g., Notepad)**.
...
2. Download the ISO file and its [.sig](https://cdn77.cachyos.org/ISO/desktop/260124/cachyos-desktop-linux-260124.iso.sig) signature file and run the following command (replace `full_iso_name.iso` with the actual ISO filename):
   ```shell
