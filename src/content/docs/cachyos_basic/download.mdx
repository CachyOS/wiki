...
2. Download the ISO file and its [.sig](https://cdn77.cachyos.org/ISO/desktop/251129/cachyos-desktop-linux-251129.iso.sig) signature file and run the following command (by replacing `full_iso_name.iso` with the actual ISO filename):
   ```shell
