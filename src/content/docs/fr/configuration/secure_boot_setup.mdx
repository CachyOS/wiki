...
3. Enregistrez vos clés avec les clés de Microsoft et micrologiciel FOE (OEM) :
    ...
    ```
    :::caution[Cartes mères ASUS — Ne pas utiliser `--firmware-builtin`]
    Sur certaines cartes mères ASUS, l'utilisation de la commande `--firmware-builtin` entraîne
    la création de doublons dans les variables de clés EFI (p.e `builtin-db` apparaît plusieurs 
    ...
    Si « Vendor Keys » affiche « microsoft builtin-db builtin-db ... », les clés doivent être
...
# Malgré le fait que Limine possède un processus de démarrage différent, dans certains cas l'erreur "Secure Boot Violation - Invalid signature detected" peut survenir. Cela est possiblement dû à BOOTX64.EFI qui n'est pas signé :
