...
### Snapshots BTRFS amorçables

:::note
Si votre installation de CachyOS date d'avant la version d'août, vous devrez peut-être activer manuellement la fonctionnalité de snapshots BTRFS amorçables.

Sinon, vous pouvez sauter cette section, car elle est activée par défaut depuis la version d'août.
:::
...
*   **Default Entry :** Spécifie quelle entrée du menu démarre par défaut. Les entrées sont numérotées à partir de 1. Si non défini, la valeur par défaut est 1.
    ...
    :::tip
    Si `default_entry` pointe vers un répertoire (par ex., ` /+CachyOS`), le démarrage automatique sera désactivé. Pour démarrer automatiquement une entrée dans un répertoire, `default_entry` doit pointer directement vers ce numéro d'entrée spécifique.
    :::
...
*   **Polices :** Utilisez un [fichier de police personnalisé](https://github.com/viler-int10h/vga-text-mode-fonts) et ajustez sa taille.
...
*   [Documentation officielle de configuration de Limine](https://github.com/limine-bootloader/limine/blob/v9.x/CONFIG.md)
