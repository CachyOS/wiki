...
:::note
Le paquet `proton-cachyos` des dépôts CachyOS est compilé avec les bibliothèques système natives ; il n'utilise pas le Steam Linux Runtime. L'architecture cible dépend du dépôt. Actuellement, les dépôts `x86_64_v3`, `x86_64_v4` et `znver4` utilisent les mêmes binaires `x86_64_v3`.
:::
...
- AMD Anti-Lag
    ...
    :::danger
    proton-cachyos désactive ceci car cela empêche les mises à jour de FSR4 et peut provoquer des saccades ou d'autres problèmes ([détails](https://github.com/CachyOS/CachyOS-PKGBUILDS/pull/909)).
    :::
...
<Tabs>
    ...
    ```shell
    WINE=/opt/wine-cachyos/bin/wine WINEPRFIX=<votre préfixe> winetricks <verbe>
    ```
    </TabItem>
...
- `proton-cachyos` (inclus dans le paquet `cachyos-gaming-meta`) est développé avec des changements améliorant la qualité de vie, des patchs triés sur le volet et des optimisations de compilation (`x86-64-v3` & `x86-64-v4`).
:::
