...
<Steps>
    1. Désactivez ZRam en ajoutant un paramètre de noyau. [Modifiez la configuration de votre chargeur d'amorçage](/fr/configuration/boot_manager_configuration) et ajoutez la ligne suivante :
        ```text
        systemd.zram=0
        ```
    2. Activez Zswap en ajoutant le paramètre de noyau suivant :
        ```text
        zswap.enabled=1 zswap.shrinker_enabled=1 zswap.compressor=lz4 zswap.max_pool_percent=30
        ```
    3. Créez un fichier d'échange (swap file) pour Zswap.
        <details>
    ...
    4. Reconstruisez votre initramfs pour appliquer les changements. La commande varie en fonction de votre chargeur d'amorçage :
        <details>
    ...
    5. Redémarrez votre système pour appliquer tous les changements.
</Steps>
