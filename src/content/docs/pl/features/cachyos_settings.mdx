...
- **`dlss-swapper`** - Skrypt opakowujący wymuszający użycie najnowszego presetu DLSS w grach obsługujących tę technologię
- **`dlss-swapper-dll`** - Działa podobnie jak `dlss-swapper`, ale wymaga ręcznej aktualizacji biblioteki `nvngx_dlss.dll` dostarczonej z grą; może działać w grach, które mają problemy ze zwykłą wersją skryptu
