...
### Mirror issues (cachyos-rate-mirrors)

Sometimes, you might encounter issues when updating your system or installing packages, such as slow downloads or errors during package retrieval. These problems can often be traced back to the package mirrors your system is configured to use. Mirrors can become outdated, slow, or temporarily unavailable.

#### Common signs you might need to update your mirror list:

* Slow package download speeds.
* Errors like error: failed retrieving file '...' from ... : The requested URL returned error: 404. While this often means your local package database is outdated (sudo pacman -Syu fixes this), it can also happen if your current mirror hasn't synchronized the latest packages yet.
* Errors like error: failed retrieving file '...' from ... : Connection timed out, Couldn't resolve host name, or other network-related failures when trying to download packages.
* Specific HTTP errors like 5xx (e.g., 526) indicating server-side problems with the mirror you are trying to reach.

#### How cachyos-rate-mirrors helps:

The cachyos-rate-mirrors command, utilizing the rate-mirrors tool, automatically tests the available CachyOS mirrors based on speed and synchronization status. It then updates your mirror list file (/etc/pacman.d/cachyos-mirrorlist) with the best-performing ones currently available. This helps ensure your system downloads packages from fast and reliable sources.

**How to run it:**

```sh
sudo cachyos-rate-mirrors
# After running this, it's recommended to refresh your package databases:
sudo pacman -Syu
```

:::tip
Running sudo cachyos-rate-mirrors periodically, especially if you notice slowdowns or errors during system updates (`pacman -Syu`), can help maintain a smooth package management experience.
:::
