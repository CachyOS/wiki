...
### A Warning About GUI Package Managers

:::caution[A Warning About GUI Package Managers]
While graphical package managers offer convenience, certain ones are known to cause severe issues on rolling-release systems like CachyOS and should be avoided for managing native system packages.

-   **Pamac:** Using Pamac as a GUI package manager is **strongly discouraged**. It is known to improperly handle package management tasks, which can break system package keyrings and lead to PGP signature errors that prevent you from updating your system.

-   **Discover (KDE) & GNOME Software Center:** These app stores use the PackageKit backend. While generally safe for managing Flatpaks, using them to install or update **native system packages** is risky. PackageKit-based managers can also be unstable or "crashy" potentially leaving your system in a broken state after a failed transaction.

For maximum stability and reliability, we **highly recommend** managing system packages through the command line with `pacman`.

If you prefer a graphical interface, GUI front-ends like **Octopi** or the **CachyOS Package Installer** are considered safe alternatives, as they are more direct wrappers for `pacman`'s functionality.
:::
