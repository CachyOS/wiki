...
1.  **Locate Your Cachy Browser Profile Folder:**
    ...
    *   Click the "Open Directory" (or similar wording like "Show in Finder" on macOS, "Open Folder" on Windows/Linux) button next to the "Root Directory". This will open the folder containing your profile data.
    *   Alternatively, you can use `about:support` and find the "Profile Folder" entry.
    *   **Typical Location on Linux:** `~/.cachy-browser/cachy-browser/<profile_name>/` or sometimes `~/.mozilla/firefox/<profile_name>/` if it used standard paths. The `<profile_name>` is usually a random string followed by `.default` or similar.
...
2.  **Locate Your Firefox Profile Folder:**
    ...
    *   **Typical Location on Linux:** `~/.mozilla/firefox/<profile_name>/`
...
6.  **Troubleshooting:**
    ...
    *   **Privacy Settings:** Cachy Browser likely had stricter privacy settings enabled by default (similar to LibreWolf). Review Firefox's privacy and security settings (`about:preferences#privacy`) and adjust them to your liking.
...
2.  **Passwords:**
    *   In Cachy Browser: Go to Settings > Privacy & Security > Logins and Passwords > Saved Logins. Click the three-dots menu (...) and choose "Export Logins...". Save the `.csv` file (be aware this file is unencrypted).
    *   In Firefox: Go to Settings > Privacy & Security > Logins and Passwords > Saved Logins. Click the three-dots menu (...) and choose "Import from a File...". Select the `.csv` file. Delete the `.csv` file securely afterwards.
