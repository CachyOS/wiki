...
1. Run `sudo passwd <new_user>` to set a password for the account
    1. If you _really_ want to use the same account (highly discouraged), instead copy the dots over to the user folder `cp -r /etc/skel/. ~` (make sure to create a backup/snapshot first!)
