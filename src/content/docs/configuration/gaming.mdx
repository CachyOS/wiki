...
:::note
Since there is some confusion happening on **Reddit**, the native version of `Proton-CachyOS` is not being deprecated as a result of `steam-native-runtime` being deprecated.

They are not related to each other. Our suggestion lately has been the Steam Linux Runtime (SLR) version, **which we also install by default**, to reduce confusion and issues for newcomers to CachyOS and Linux in general. If you prefer the native version, feel free to keep using it.
:::
...
:::note
For new users, we suggest using Valve's Proton or Proton Experimental. If you want to experiment with the extra features, want to use Proton outside of Steam, or need fixes not present in the official Proton, here are a few custom Proton flavors you can try.
:::

- `Proton-CachyOS` is the one built and maintained by [CachyOS](https://github.com/CachyOS/proton-cachyos). Using it over `Proton Experimental` is recommended **only** if you want the extra quality-of-life features and fixes. You can find two flavors in the CachyOS repositories: [proton-cachyos-native](https://packages.cachyos.org/package/cachyos/x86_64/proton-cachyos-native) and [proton-cachyos-slr](https://packages.cachyos.org/package/cachyos/x86_64/proton-cachyos-slr). We suggest using the `proton-cachyos-slr` package over the `proton-cachyos-native` one.
- `Proton-EM` is a development oriented custom build made by [Etaash-mathamsetty](https://github.com/Etaash-mathamsetty/Proton). It is a very good option if you want to use `winewayland` and `FSR4` without any of the extras `Proton-CachyOS` offers. It is also the source of various improvements that `Proton-CachyOS` also includes. If you are interested in the latest and greatest in `winewayland` development, this is for you.
- `dwproton` is a custom build made by [Dawn Winery](https://dawn.wine/dawn-winery/dwproton), it is a fork of `Proton-CachyOS` with extra fixes and patches for certain anime games. If you encounter issues with such games with other Proton flavors, `dwproton` is your best option.
