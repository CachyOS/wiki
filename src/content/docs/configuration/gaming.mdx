...
:::caution[NTSync is not enabled by default]
You can enable it by setting the environment variable `PROTON_USE_NTSYNC=1`, or disable it by setting `PROTON_USE_NTSYNC=0`
:::
...
- HDR Support
    - `PROTON_ENABLE_HDR=1`: Enable HDR output support.
      - **Requirements**: Gamescope with `--hdr-enabled` OR `PROTON_ENABLE_WAYLAND=1`. If your GPU is **NVIDIA**, add `ENABLE_HDR_WSI=1` and install the `vk-hdr-layer-kwin6-git` package.
