...
- HDR Support
    - `PROTON_ENABLE_HDR=1`: Enable HDR output support.
      - **Requirements**: Gamescope with `--hdr-enabled` OR `PROTON_ENABLE_WAYLAND=1`. If your GPU is **NVIDIA**, add `ENABLE_HDR_WSI=1` and install the `vk-hdr-kwin6-git` package.
