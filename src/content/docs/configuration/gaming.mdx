...
:::caution[NTSync is not enabled by default. You can enable it using the `PROTON_USE_NTSYNC=1` environment variable.]
:::
...
- CPU & Sync Performance
    - `PROTON_USE_NTSYNC=1`: Use NTSync instead of WINESync.
        - **Benefit**: Better CPU performance
        - **Best for**: CPU-bound games
...
:::tip
CachyOS provides various Proton versions that are built & maintained for further performance improvements:
- `proton-cachyos` (included in the `cachyos-gaming-meta` package) is developed with added QoL changes, cherry-picked patches, and compilation optimizations (`x86-64-v3` & `x86-64-v4`).
:::
...
:::note
`gamemode` and `game-performance` are both different tools. [gamemode](https://github.com/FeralInteractive/gamemode) is a tool made by Feral Interactive, while `game-performance` is a custom script made by CachyOS.
:::
