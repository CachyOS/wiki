...
:::note
Custom protons have unstable configuration options by their nature. For the latest information on their respective configuration options refer to their documentation.
...
:::note
Since there is some confusion going on **Reddit**, the native version of `Proton-CachyOS` is not being deprecated as a result of `steam-native-runtime` that has been deprecated.

They are not related and are irrelevant to each other. Our suggestion lately has been the Steam Linux Runtime (SLR) version, **which we also install by default**, to reduce confusion and issues for newcomers to CachyOS and Linux in general. If you like the native version, and it works for you, keep using it.
:::
...
- `Proton-CachyOS` is the one built and maintained by [CachyOS](https://github.com/CachyOS/proton-cachyos). Using it over `Proton Experimental` is recommended **only** if you want the extra quality-of-life features and fixes. In the CachyOS repositories you can find two different flavors if it, [proton-cachyos-native](https://packages.cachyos.org/package/cachyos/x86_64/proton-cachyos-native) and [proton-cachyos-slr](https://packages.cachyos.org/package/cachyos/x86_64/proton-cachyos-slr). We suggest using the `proton-cachyos-slr` package over the `proton-cachyos-native` one.
...
- `dwproton` is a custom build made by [Dawn Winery](https://dawn.wine/dawn-winery/dwproton) is a fork of `Proton-CachyOS` with extra fixes and patches for certain anime games. If you encounter issues with such games with other Proton flavors, `dwproton` is your best option.
