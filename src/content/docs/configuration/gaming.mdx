...
<Tabs>
    ...
    5. Add the following environment variables:
        - **Key**: `UMU_RUNTIME_UPDATE` *optional*
            - **Value**: `0` *skips Steam Linux Runtime updates for `proton-cachyos.`* **Do not use with any Proton that uses the Steam Linux Runtime, e.g. proton-cachyos-slr, -GE and -EM** 
...
If you are configuring a gaming only PC you may find it simpler to change the Power Management settings to be permanently in performance mode, to
disable Suspend Session, screen dimming on inactivity and in Screen Locking to disable automatic screen locking. Read below to configure 
power management on game lauch instead.
