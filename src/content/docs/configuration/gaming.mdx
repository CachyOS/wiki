...
How to add & make game-performance script launch with any wine instance.

On the right, open the hamburger menu > Preferences > Runners > Scroll down to Wine and click on the toolbox icon > Enable advanced mode on the top right > System options > Pre-launch script > either type or point to the following: /usr/bin/game-performance > Save changes 
