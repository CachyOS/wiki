...
The order of launch options in Steam follows the following order
- `<env variables> <wrappers> %command% <application arguments>`
    ...
    - `<wrappers>`: Applications and scripts that modify how the real application is run. Arguments to the wrapper usually go after the executable. For example `mangohud --dlsym` or `gamescope -W 1680 -H 1050 -w 1280 -h 720 -S fit -F fsr --mangoapp --`
    - `%command%`: This is the real application. This should be specified exactly as is and Steam will replace it with the actuall command when the application is run.
    - `<application arguments>`: These are various arguments to the actual application, and they depend on the application. For example `-dx11` should go here if the application supports it.
