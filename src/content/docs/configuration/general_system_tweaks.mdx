...
# Install lsp-plugins for additional effects and jalv for LV2 plugin support
...
There are many packages providing the Obs Studio but the recommended one to use
is the obs-studio-browser. This is patched for smooth installation and fixes
issues like cuda-error, virtual camera not working etc. It can be installed by
searching obs-studio-browser in the Cachy OS package installer or by typing the
command. Make sure that another obs package if installed is removed. ''' bash
#Install obs-studio browser
sudo pacman -S obs-studio-browser
'''.
It may require a restart to take effect.
