...
Check which mitigation's affects your CPU by running the following command:
...
CachyOS earlier used as default uksmd (userspace kernel samepage merging) and then we replaced it by the MemoryKSM function provided by systemd.
Since there is some CPU overhead and mainly benefits Hosts which are running VM's. We decided to disable this feature for now.

To reactivate it, execute the following command:
```sh
sudo ksmctl --enable
```

Once activated KSM will start merging identical memory pages into one **"if possible"**.

To disable KSM, run the following command:
```sh
sudo ksmctl --disable
```
