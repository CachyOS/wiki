...
```
    ...
    <Tabs>
        ...
        <TabItem label='Starting & Switching with custom arguments'>
          ```sh title='Starting scx_rustland with custom arguments'
          scxctl start --sched rustland --args="-m performance -w"
          ```
          ```sh title='Switching to scx_lavd with custom arguments'
...
**Low Latency**
* **Command-line Flags:** `-y`
* **Description:** Lowers latency by making interactive tasks stick more to the CPU they were assigned to.
