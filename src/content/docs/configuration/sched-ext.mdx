...
- Use cases:
  ...
  - Gaming: Works surprisingly well in certain games, although your mileage may vary.
...
scx_forge contains two parts:

1. A base AI-Oriented scheduler that is designed to be used in conjunction with the second part.
2. Agent intended to be used with the base scheduler to provide a more AI-oriented scheduling experience, plus optimization in place by the agent itself.

The agent allows setting up your AI model, workload you want to optimize for and the overall goal you want to achieve. The agent will then try to optimize the scheduler for your specific workload and goal.

How? via the [spec.toml file](https://github.com/sched-ext/scx/blob/main/tools/scx_forge_agent/spec.toml) which is used to start the optimization loop.

Basically this scheduler is perfect for those who want to optimize an specific workload by an AI agent.

To learn more about it, check the [scx_forge_agent README](https://github.com/sched-ext/scx/tree/main/tools/scx_forge_agent).
