...
- **`dlss-swapper`** - Скрипт-обёртка для принудительного включения последней версии DLSS в играх, поддерживающих эту технологию
- **`dlss-swapper-dll`** - Аналогичен `dlss-swapper`, но требует ручного обновления библиотеки `nvngx_dlss.dll`, поставляемой с игрой; может работать с играми, в которых возникают проблемы с обычной версией скрипта
