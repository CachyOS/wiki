---
title: Помощник chroot от CachyOS
description: Вспомогательный инструмент для упрощения входа в системы через chroot
tableOfContents:
  minHeadingLevel: 1
  maxHeadingLevel: 4
---
...
[cachy-chroot](https://github.com/CachyOS/cachy-chroot) — это простая вспомогательная программа для облегчения процесса входа через chroot в существующую
установку CachyOS или дистрибутива на основе Arch. Она выводит список всех обнаруженных на машине разделов, а также поддерживает отображение подтомов BTRFS.
...
Символ **❯** указывает на текущую выбранную опцию.

**?** указывает на запрос ввода от пользователя.
:::

<Steps>
1. Загрузитесь с live ISO CachyOS.
2. Откройте терминал и войдите в систему как root-пользователь с помощью `sudo su`.
3. Убедитесь, что у вас установлена последняя версия `cachy-chroot`, выполнив команду:
    ```bash
...
4. Запустите `cachy-chroot`, набрав:
    ...
    <Tabs>
        <TabItem label='Пример вывода с установкой CachyOS на BTRFS'>
            ```bash
        ...
        <TabItem label='Пример вывода с EXT4'>
            ```bash
...
5. Выберите раздел, который содержит корневую файловую систему:
    <Tabs>
        <TabItem label='Пример с CachyOS BTRFS'>
            ...
            ```
            **Если вы используете CachyOS с BTRFS**, введите `y`, чтобы использовать предустановку CachyOS BTRFS. Это автоматически смонтирует корневой подтом и другие важные подтома, такие как `/home`, `/var`, `/tmp` и `/srv`. Если вы используете пользовательскую разметку BTRFS или систему, отличную от CachyOS, введите `n` для ручного выбора подтомов.
        </TabItem>
        <TabItem label='Пример с EXT4'>
            ```bash title="Выбор корневого раздела"
...
6. `cachy-chroot` попытается автоматически смонтировать все разделы и подтома, перечисленные в `/etc/fstab` корневого устройства. Если какие-либо разделы не смогут смонтироваться, вы получите уведомление и возможность смонтировать их вручную при необходимости. Вы можете выбрать `no`, чтобы пропустить монтирование дополнительных разделов.
...
8. Когда закончите, выйдите из chroot-окружения, введя `exit` в командной строке или нажав `CTRL+D` на клавиатуре.
    ```bash title="Выход из chroot"
    exit
    ```
9. После выхода `cachy-chroot` автоматически очистит смонтированные разделы и закроет все LUKS-контейнеры. Вы вернетесь в оболочку live-окружения.
</Steps>

### Устранение неполадок

- **Разделы не найдены:** Убедитесь, что диск с вашей установкой подключен и распознается live-системой. Вы можете проверить это с помощью `lsblk` или `fdisk -l`.
- **Автоматическое монтирование некоторых разделов не удалось:** Вы можете попробовать смонтировать их вручную из chroot-окружения. `cachy-chroot` проигнорирует сбои и продолжит работу.

### FAQ

- **В: Зачем монтировать дополнительные разделы?**
...
- [Arch Wiki - chroot](https://wiki.archlinux.org/title/Chroot) (на английском)
