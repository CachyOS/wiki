...
Bcachefs — это новая передовая файловая система для Linux, с упором на надёжность и отказоустойчивость, а также с полным набором функций, ожидаемых от современной файловой системы.
...
- Копирование при записи (CoW) — как у BTRFS или ZFS
- Сжатие
...
`bcachefs-dkms` обеспечивает поддержку в виде модуля ядра, не входящего в его состав.
`bcachefs-tools` для пользовательских утилит.
