...
CachyOS предлагает 5 различных файловых систем, чтобы позволить пользователю выбрать ту, что наилучшим образом соответствует его потребностям. Далее будут рассмотрены преимущества, недостатки и рекомендации для каждой файловой системы. Все необходимые зависимости/утилиты для каждой файловой системы уже предустановлены в CachyOS.
...
Пакет, содержащий инструменты userspace для управления файловыми системами XFS, называется `xfsprogs`.
...
Пакет с утилитами userspace для Btrfs называется `btrfs-progs`.
...
`zfs-utils` для утилит userspace.
