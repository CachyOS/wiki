...
[sbctl](https://github.com/Foxboron/sbctl) — это удобный менеджер ключей безопасной загрузки, способный настраивать безопасную загрузку,
...
Если вы используете GRUB, выполните следующую команду, чтобы включить поддержку безопасной загрузки в GRUB с использованием CA-ключей.
...
Сначала нам нужно зайти в настройки прошивки и установить для режима безопасной загрузки значение "Setup Mode" (Режим настройки). Вы можете перезагрузиться из
...
sudo sbctl create-keys # Создание ваших собственных ключей безопасной загрузки
Created Owner UUID a9fbbdb7-a05f-48d5-b63a-08c5df45ee70
Creating secure boot keys...✔
Secure boot keys created!

sudo sbctl enroll-keys --microsoft # Регистрация ваших ключей вместе с ключами Microsoft
...
Пользователям Limine следует перейти к разделу [Limine](#limine).
...
:::caution
В системах с отдельными разделами `/boot` и `/boot/efi` `sbctl` может сканировать EFI-файлы только в `/boot/efi`.
...
Теперь, когда все файлы подписаны, мы можем перезагрузиться в настройки UEFI и включить безопасную загрузку.
...
- [sbctl](https://github.com/Foxboron/sbctl) - Это простое руководство по включению поддержки безопасной загрузки не было бы возможным, если бы не
