...
:::note
Das `proton-cachyos`-Paket aus den CachyOS-Repositories wird gegen native Systembibliotheken kompiliert; es nutzt nicht die Steam Linux Runtime. Die Zielarchitektur hängt vom Repository ab. Aktuell verwenden die `x86_64_v3`-, `x86_64_v4`- und `znver4`-Repositories dieselben `x86_64_v3`-Binärdateien.
:::
...
- Wayland & Anzeige
    - `PROTON_ENABLE_WAYLAND=1`: Aktiviert native Wayland-Unterstützung.
      - **Vorteile**: Ermöglicht HDR ohne Gamescope; verbessert Latenz/Frame-Pacing
      - **Nachteile**: Macht das Steam-Overlay kaputt, derzeit experimentell
...
- HDR-Unterstützung
    - `PROTON_ENABLE_HDR=1`: Aktiviert die HDR-Ausgabeunterstützung.
      - **Anforderungen**: Gamescope mit `--hdr-enabled` ODER `PROTON_ENABLE_WAYLAND=1`
...
- CPU- & Sync-Performance
    - `PROTON_USE_NTSYNC=1`: Nutzt NTSync anstelle von WINESync.
        - **Vorteil**: Mögliche Steigerung der Flüssigkeit in bestimmten Spielen
        - **Hinweis**: Experimentell, kann in manchen Spielen Probleme verursachen
        - **Überprüfung**: Nutze `lsof /dev/ntsync` (MangoHud meldet es fälschlicherweise)

- Shader- & Cache-Verwaltung
    - `PROTON_LOCAL_SHADER_CACHE=1`: Aktiviert einen spielspezifischen Shader-Cache, ähnlich wie bei Steam's "Shader Pre-Caching".
        - **Hinweis**: Dies kompiliert die Shader NICHT im Voraus, es isoliert NUR den Shader-Cache jedes Spiels. Shader werden weiterhin während des Spielens kompiliert.
...
- Controller & Eingabe
    ...
    - `PROTON_NO_STEAMINPUT=1`: Deaktiviert die Steam-Input-Unterstützung.
        - **Behebt**: Wayland-Probleme mit Controllern/Gamepads
...
- `proton-cachyos` (im Paket `cachyos-gaming-meta` enthalten) wird mit zusätzlichen Quality-of-Life-Änderungen, ausgewählten Patches und Kompilierungsoptimierungen (`x86-64-v3` & `x86-64-v4`) entwickelt.
:::

:::tip
Um zu prüfen, ob ein Spiel mit Linux kompatibel ist oder wie es unter Linux läuft, besuche **[ProtonDB](https://www.protondb.com/)** oder **[Are We Anti-Cheat Yet?](https://areweanticheatyet.com/)**.
:::
...
3. Füge Folgendes am Ende der Datei ein:
	```sh
	# Erhöhe Nvidias Shader-Cache-Größe auf 12 GB
...
3. Füge Folgendes am Ende der Datei ein:
	...
	# Erhöhe AMDs Shader-Cache-Größe auf 12 GB
