...
:::caution
Auf Systemen mit getrennten Partitionen für `/boot` und `/boot/efi` sucht `sbctl` möglicherweise nur in `/boot/efi` nach EFI-Binärdateien.
Das führt dazu, dass Kernel-Images, die sich in `/boot` befinden, nicht erkannt werden. `sbctl-batch-sign` umgeht dies, indem es **immer**
`/boot` nach `vmlinuz-*`-Dateien durchsucht.
:::

Nachdem nun alle Dateien signiert sind, können wir zurück in die UEFI-Einstellungen neustarten und Secure Boot aktivieren.
...
alle neuen Dateien bei einem Kernel- oder Boot-Manager-Update signiert.
