...
Boot-Prozess hat, der das EFI-Chainloading und die Signaturprüfungen umgeht. Die einzige EFI-
Binärdatei, die signiert werden muss, ist Limine selbst.
...
Um `fwupd` mit aktiviertem Secure Boot zu verwenden, signiere die UEFI-Binärdatei:

```sh
sudo sbctl sign -s -o /usr/lib/fwupd/efi/fwupdx64.efi.signed /usr/lib/fwupd/efi/fwupdx64.efi
```

Und füge folgendes zu `/etc/fwupd/fwupd.conf` hinzu:
