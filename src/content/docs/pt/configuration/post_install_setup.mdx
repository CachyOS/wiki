...
## Ativar suporte AppArmor utilizando perfis AppArmor.d (Opcional)

<Steps>

1. Adicione os seguintes parâmetros de kernel ao seu Gestor de Arranque (Boot Manager), consulte [Configuração do Gestor de Arranque](/configuration/boot_manager_configuration) para referência:

   ```text
   lsm=landlock,lockdown,yama,integrity,apparmor,bpf
   ```

2. Instale os pacotes `apparmor` e `apparmord` **(Conjunto de mais de 1500 perfis)**
   ```bash
   sudo pacman -S apparmor apparmor.d
   ```

3. Ativar/Iniciar o serviço AppArmor

   ```bash
   systemctl enable --now apparmor.service
   ```

4. Ativar o cache para os perfis do AppArmor
...
Para gerir AppImages, pode utilizar o [AppImageLauncher](https://github.com/TheAssassin/AppImageLauncher), que oferece uma forma fácil de integrar AppImages no seu sistema.
<Tabs>
  <TabItem label='Utilizar o AppImageLauncher (GUI)'>
    O AppImageLauncher é uma ferramenta gráfica que simplifica a gestão de AppImages no seu sistema. Integra-se com o seu ambiente de trabalho, facilitando a execução e gestão de AppImages.
        <Steps>
        1. Instale o AppImageLauncher:
              ```sh
              paru appimagelauncher
              ```
        2. Descarregue uma AppImage da sua escolha a partir de uma fonte fidedigna.

        3. Clique duas vezes no ficheiro AppImage descarregado. O AppImageLauncher irá perguntar se deseja integrar a aplicação no seu sistema.

        4. Siga as instruções para concluir o processo de integração.

        5. Uma vez integrada, poderá iniciar a aplicação a partir do menu de aplicações ou clicando duas vezes no ficheiro AppImage.
        </Steps>
    </TabItem>
</Tabs>
