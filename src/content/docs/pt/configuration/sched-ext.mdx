...
### Existem tantas opções... por que razão não criam apenas um scheduler que seja bom em tudo?

A razão principal é que não existe uma solução única que sirva para todos os casos no que toca ao escalonamento (scheduling) do CPU. Diferentes cargas de trabalho têm diferentes requisitos e prioridades, e um scheduler que esteja otimizado para um tipo de carga pode não ter um bom desempenho noutro.

Podes ver os schedulers de CPU como diferentes pares de sapatos, cada um desenhado para uma atividade específica. Por exemplo, correr um jogo num scheduler que esteja otimizado para cargas de servidor pode levar a um desempenho abaixo do esperado e a um aumento da latência, enquanto usar um scheduler desenhado para gaming num servidor pode resultar numa utilização ineficiente de recursos e numa redução da capacidade de processamento (throughput).

Essa é a magia do sched-ext. As limitações deixaram de ser um problema.
