...
## Recuperação do Carregador de Arranque e Snapshots Btrfs
...
### Utilizar um snapshot Btrfs como ponto de restauro (rollback)

:::note
Embora o CachyOS crie automaticamente snapshots Btrfs antes de cada atualização do sistema através de hooks do pacman, é sempre recomendável criar os seus próprios snapshots antes de efetuar alterações importantes no sistema.
:::

Para mais informações sobre snapshots Btrfs, consulte a [documentação do Btrfs.](https://btrfs.readthedocs.io/en/latest/)

Um snapshot BTRFS aparece como uma entrada de arranque adicional no menu do seu carregador de arranque (bootloader) e geralmente tem um nome semelhante a:
- `10 | 10-30-2025 14:37:10`

<details>
<summary>Exemplo numa captura de ecrã:</summary>
<ImageComponent imgsrc={import('~/assets/images/btrfs-snapshot-boot-entry.jpg')} />
</details>

Também pode utilizar a aplicação Btrfs Assistant para gerir os seus snapshots. Esta oferece uma interface gráfica para criar, eliminar e restaurar snapshots.

<details>
    <summary>Captura de ecrã do Btrfs Assistant:</summary>
