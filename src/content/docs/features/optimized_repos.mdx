...
2. Install the necessary packages:
   ...
   ```
   :::note
   At the moment, only CachyOS's pacman fork is able to recognize and automatically check for the optimized architectures.
   If you chose to skip installing the customized pacman in this step, pacman has to be configured to recognize the appropriate architecture(s) manually.
   For example:

   ```ini
   # /etc/pacman.conf
   [options]
   Architecture = auto x86_64_v3 x86_64_v4
   ```
   :::
