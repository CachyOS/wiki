...
## Migrating from x86-64-v3 to Zen 4 repositories

If you are currently using the `x86-64-v3` repositories and you upgrade to a Zen 4 or Zen 5 CPU, you can migrate to our `znver4` repositories to take advantage of the additional optimizations available for your CPU architecture.

<Steps>

1. Make sure your CPU is based on Zen 4 or Zen 5. You can check this by running the following command:
   ```sh
...
2. Add the `znver4` repositories to your `/etc/pacman.conf` file as shown below:
   ...
   Include = /etc/pacman.d/mirrorlist
    ...
    ```
3. Clear the package cache and synchronize the package databases:
   ```sh
   sudo pacman -Scc # Enter Y in both instances
   sudo pacman -Sy
   ```
4. Reinstall all packages to switch to the `znver4` versions:
   ```sh
