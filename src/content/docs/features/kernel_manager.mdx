...
* **Description:** Lowers CPU busy threshold and disables polling to improve responsiveness.

**Power Save**
* **Command-line Flags:** `-m powersave -d -p 5000`
* **Description:** Prioritizes power efficiency. Favor less performant cores (e.g., E-cores on Intel) and disables deferred wakeups to reduce throughput while increasing power efficiency. Polling increased to 5ms

**Low Latency**
* **Command-line Flags:** `-m performance -c 0 -p 0 -w`
* **Description:** Meant to lower latency at the cost of throughput. Suitable for soft real-time applications like Audio Processing and Multimedia. Disables polling and wake up sync to reduce latency.
