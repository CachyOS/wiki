---
title: Descargas y Validación
description: Descarga y validación de la ISO de CachyOS.
---

import { Steps, Tabs, TabItem } from '@astrojs/starlight/components';

## Descargando la ISO

La ISO de CachyOS se puede obtener de las siguientes fuentes:

*   [Sitio web](<https://cachyos.org/download>)
*   [SourceForge](<https://sourceforge.net/projects/cachyos-arch/files/>)
*   [Mirror de CachyOS](<https://mirror.cachyos.org/ISO/>)

## Verificando la integridad de la ISO con SHA256

:::caution[ADVERTENCIA]
Tómese siempre el tiempo de verificar la integridad de la ISO para evitar problemas no deseados durante la instalación o al crear un medio de arranque.
:::

**Versión actual de la ISO:** `250828`

**Hash SHA256:** `83ab50783ed41bff0f497b223a78312edef7d4d718f22baf7a6b01a6eb85d88b`

<Tabs>

<TabItem label='Windows'>

<Steps>

1. Descargue el [archivo](https://mirror.cachyos.org/ISO/desktop/250828/cachyos-desktop-linux-250828.iso.sha256) que contiene el hash SHA256 **(Ábralo con un editor de texto, por ejemplo: Bloc de notas).**
2. Abra CMD o PowerShell como Administrador y [navegue](<https://www.wikihow.com/Change-Directories-in-Command-Prompt>) a la ruta donde se guardan los archivos ISO y SHA256.
3. Ejecute el siguiente comando:
   ```powershell
   # Ejemplo:
   certUtil -hashfile cachyos-desktop-linux-250828.iso SHA256
   ```
4. Compare la salida del hash de certUtil con la del archivo descargado en el **Paso 1.** Si coinciden, puede proceder con la instalación de CachyOS.

</Steps>

</TabItem>

<TabItem label='Linux'>

<Steps>

1. Descargue el [archivo](https://mirror.cachyos.org/ISO/desktop/250828/cachyos-desktop-linux-250828.iso.sha256) que contiene el hash SHA256.
2. Abra una terminal y navegue al directorio que contiene el archivo `.sha256` y ejecute los siguientes comandos:
   ```sh
   # Ejemplo:
   cd Descargas/ || exit
   cat cachyos-desktop-linux-250828.iso.sha256
   # 83ab50783ed41bff0f497b223a78312edef7d4d718f22baf7a6b01a6eb85d88b
   ```
3. Compare la salida del **Paso 2** y ejecute el siguiente comando para verificar cuál es el hash actual en el archivo ISO.
   ```sh
   # Ejemplo:
   sha256sum cachyos-desktop-linux-250828.iso
   ```
4. Si los hashes del **Paso 2** y del **Paso 3** coinciden, puede proceder con la instalación de CachyOS.

</Steps>

</TabItem>

<TabItem label='macOS'>

<Steps>

1. Descargue el [archivo](https://mirror.cachyos.org/ISO/desktop/250828/cachyos-desktop-linux-250828.iso.sha256) que contiene el hash SHA256.
2. Abra una terminal y navegue al directorio que contiene el archivo `.sha256` y ejecute los siguientes comandos:
   ```sh
   # Ejemplo:
   cd Downloads/ || exit
   cat cachyos-desktop-linux-250828.iso.sha256
   # 83ab50783ed41bff0f497b223a78312edef7d4d718f22baf7a6b01a6eb85d88b
   ```
3. Compare la salida del **Paso 2** y ejecute el siguiente comando para verificar cuál es el hash actual en el archivo ISO.
   ```sh
   # Ejemplo:
   shasum -a 256 cachyos-desktop-linux-250828.iso
   ```
4. Si los hashes del **Paso 2** y del **Paso 3** coinciden, puede proceder con la instalación de CachyOS.

</Steps>

</TabItem>

</Tabs>

## Verificar la Autenticidad de la Imagen ISO (Linux)

Para verificar la autenticidad del archivo ISO y asegurarse de que es el publicado por el equipo de desarrollo oficial de CachyOS:

<Steps>

1. Importe la clave GPG para verificar la autenticidad:

   ```shell
   gpg --keyserver hkps://keys.openpgp.org --recv-key F3B607488DB35A47
   ```

2. Descargue el archivo ISO y su archivo de firma `.sig` y ejecute el siguiente comando (reemplazando `nombre_completo_iso.iso` por el nombre real del archivo ISO):
   ```shell
   gpg --verify nombre_completo_iso.iso.sig nombre_completo_iso.iso
   ```

   Si obtiene una salida de **Good signature** (Firma correcta), el archivo ISO es genuino:
   ```
   gpg: Signature made jue 28 ago 2025 10:41:14 -03
   gpg:                using RSA key 882DCFE48E2051D48E2562ABF3B607488DB35A47
   gpg: Good signature from "CachyOS <admin@cachyos.org>" [unknown]
   gpg: WARNING: This key is not certified with a trusted signature!
   gpg:          There is no indication that the signature belongs to the owner.
   Primary key fingerprint: 882D CFE4 8E20 51D4 8E25  62AB F3B6 0748 8DB3 5A47
   ```

</Steps>

:::danger
Si la salida no devuelve una cadena de **Good signature** (Firma correcta) o el ID de la clave no coincide, no utilice la imagen ISO. Asegúrese de que la imagen descargada provenga
de una fuente legítima de CachyOS, ya que una firma incorrecta podría sugerir que la ISO ha sido manipulada.
:::
