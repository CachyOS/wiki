...
description: Herramienta de ayuda para facilitar el proceso de hacer chroot en sistemas
tableOfContents:
  minHeadingLevel: 1
  maxHeadingLevel: 4
---
...
[cachy-chroot](https://github.com/CachyOS/cachy-chroot) es un programa de ayuda simple para facilitar el proceso de hacer chroot en una instalación existente de CachyOS o basada en Arch. Lista todas las particiones descubiertas en la máquina y también soporta listar subvolúmenes BTRFS.
Por último, pero no menos importante, `cachy-chroot` también soporta sistemas cifrados a través de LUKS. Mapeará cada entrada de `fstab` a sus entradas designadas en `crypttab`
...
:::note
cachy-chroot soporta el montaje de prácticamente cualquier sistema de archivos soportado por el kernel de Linux, excepto ZFS.
:::
...
<Steps>
1. Arranca en un ISO live de CachyOS
2. Abre una terminal e ingresa como usuario root con `sudo su`
...
4. Ejecuta `cachy-chroot` escribiendo:
    ...
    <Tabs>
        <TabItem label='Ejemplo de salida con instalación BTRFS de CachyOS'>
            ```bash
        ...
        <TabItem label='Ejemplo de salida con EXT4'>
            ```bash
...
5. Selecciona la partición que contiene el sistema de archivos raíz:
    <Tabs>
        <TabItem label='Ejemplo con BTRFS de CachyOS'>
            ...
            ? Do you want to use CachyOS BTRFS preset to auto mount root subvolume? (y/n) › # Ingresa 'yes' si estás en CachyOS
            ```
            **Si usas CachyOS con BTRFS**, ingresa `y` para usar el preajuste BTRFS de CachyOS. Esto montará automáticamente el subvolumen raíz y otros subvolúmenes importantes como `/home`, `/var`, `/tmp` y `/srv`. Si estás usando una configuración BTRFS personalizada o un sistema que no es CachyOS, ingresa `n` para seleccionar los subvolúmenes manualmente.
        </TabItem>
        <TabItem label='Ejemplo con EXT4'>
            ```bash title="Seleccionando la partición raíz"
...
8. Cuando termines, sal del entorno chroot escribiendo `exit` en la consola o presionando `CTRL+D` en el teclado.
...
- **P: ¿Para qué sirve montar particiones adicionales?**
  - R: Hay varios casos de uso para montar manualmente particiones adicionales, incluyendo:
    - **Archivo fstab dañado o faltante:** Puedes montar manualmente las particiones necesarias (como `/boot` o `/home`) para realizar reparaciones o recuperación de datos.
    - **UUIDs de partición actualizados:** Si tu sistema no arranca porque cambiaste el UUID de una partición, puedes usar `cachy-chroot` para montar las particiones y luego actualizar el fstab correspondientemente.
