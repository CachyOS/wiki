...
Welcome to the guide for gaming on CachyOS! This will guide you through everything you need to know to set up your system for gaming.

*First of all, understand that getting a double-digit improvement in FPS is not always possible. Sometimes, optimizations can lead to minor improvements or even none at all, depending on the game and hardware configuration.*

*You can't expect software optimizations to act like a free hardware upgrade.*
...
To make setting up CachyOS for gaming easier, all the necessary gaming packages are grouped into one meta-package that includes all the necessary dependencies and libraries for gaming in Linux, along with a separate meta-package for popular tools and stores/launchers.
...
You can disable it by setting the environment variable `PROTON_NO_NTSYNC=1`
:::
...
Links to Proton-CachyOS documents:
  ...
  <TabItem label='Lutris Global'>
    <Steps>
      ...
      2. Go to the **Runner Options** tab and confirm that your settings match the following:
          - **Wine version** = `proton-cachyos-slr` or `proton-cachyos-native`
...
<details>
  ...
  <Steps>
    1. Download the latest version [here](https://github.com/CachyOS/proton-cachyos/releases) (scroll down to **Assets**).
        :::note
        Choose to download the one ending with `x86-64`. It is the version most users should prefer.
        Choose the one that ends with `x86-64_v3` only if your CPU supports **[AVX2](/installation/installation_prepare#x86_64-microarchitecture-level-support)** and you want to experiment.
        :::
...
This is the same `Wine` that is at the core of `Proton-CachyOS`, but as a standalone package. It can be used in Lutris, Heroic, Bottles, and others. **Keep in mind that it is not intended to be used for Steam.**
...
- `proton-cachyos` (included in the `cachyos-gaming-meta` package) is developed with added QoL changes, cherry-picked patches, etc.
:::

:::tip
To check if a game is compatible with Linux, or how it runs on Linux, visit
**[ProtonDB](https://www.protondb.com/)** ,
**[Are We Anti-Cheat Yet?](https://areweanticheatyet.com/)**
or **[Gaming On Linux Anti Cheat Section](https://www.gamingonlinux.com/anticheat/)**
:::

### How to set custom Proton versions in Steam

:::caution[It's STRONGLY RECOMMENDED to keep Valve's Proton as the global default.]
:::

- To set a custom Proton for a specific game, navigate to Library, right-click the game
go into **Properties** > **Compatibility**

check `Force the use of a specific Steam Play compatibility tool` checkbox
...
:::tip
For new users, we suggest using Valve's Proton or Proton Experimental.
:::
...
**Proton 10.0 or lower:** are the stable releases from **Valve**. *Use this if the game you want to play only works with a previous Proton release.*

:::tip
For more experienced users, who want to experiment with extra features, use Proton outside of Steam, or need fixes not present in Valve's Proton, there are  more than a few custom Proton flavors they can use. Keep in mind that they are not supported by Valve, they are community efforts and they are experimental in their nature.
:::

Third-party Proton forks:

**Proton-CachyOS:**
Built and maintained by [CachyOS](https://github.com/CachyOS/proton-cachyos), it contains extra quality-of-life features, fixes and experimental changes.

You can find two flavors in the CachyOS repositories: [`proton-cachyos-native`](https://packages.cachyos.org/package/cachyos/x86_64/proton-cachyos-native) and [`proton-cachyos-slr`](https://packages.cachyos.org/package/cachyos/x86_64/proton-cachyos-slr).

We suggest using the `proton-cachyos-slr` package over the `proton-cachyos-native` one.

**Proton-EM:**
A development oriented custom build made by [Etaash-mathamsetty](https://github.com/Etaash-mathamsetty/Proton). It is a very good option if you want to use **winewayland** and **FSR4** without any of the extras Proton-CachyOS offers. It is also the source of various improvements that Proton-CachyOS includes.

*If you are interested in the latest and greatest in **winewayland** development, this is for you.*

**dwproton:**
A custom build made by [Dawn Winery](https://dawn.wine/dawn-winery/dwproton), it is a fork of Proton-CachyOS with **extra fixes and patches for certain anime games**.

If you encounter issues with such games with other Proton flavors, **dwproton** is your best option.

**Proton-GE**:
A custom build made by [GloriousEggroll](https://github.com/GloriousEggroll/proton-ge-custom). It includes various fixes, and can be useful to have in certain situations.

### Fix H264 videos not playing or displaying color bars in games with Valve's Proton
:::caution
This is optional. Only use it if you are experiencing the issue.
:::
Start Steam with the following command. Wait for it to load, then exit Steam and start it normally.
...
If you have time to spare, then you can ignore this tip and let Steam pre-compile some shaders for your game.
...
### Boot into gamescope session

Gamescope Session launches your desktop inside gamescope, providing a dedicated fullscreen gaming environment with improved display handling, input management, and performance optimizations.

It is ideal for gaming-focused systems or handheld devices, offering a console-like experience while still allowing access to your desktop session when needed.

To use it install `gamescope-session-cachyos` package (`pacman -S gamescope-session-cachyos`) and set the default session to gamescope.
...
- Add any game you want by clicking the **+** in the top-left corner.
...
- Environment variables such as `DXVK_HDR=1` should be added in the **Environment variables** table under the **System options** tab using the `+` button to add a new entry.
:::note
