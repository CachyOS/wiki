...
Completely removing macOS carries risks, like data loss or rendering the device unusable. It's **strongly recommended** to keep a macOS partition for firmware updates and recovery. Refer to the [t2linux wiki](https://wiki.t2linux.org/roadmap/#can-i-completely-remove-macos) for details.
