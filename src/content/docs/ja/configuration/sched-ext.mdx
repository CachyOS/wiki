...
scx_flow には Gaming, Power Save, Low Latency といったモードは**ありません**。また初期のバージョンと違い、適応的な制御ループもなくなっています。ローテーションするティアのディスパッチとバジェットのしきい値はコンパイル時の定数になっており、環境に左右されない、決定論的で予測可能な動作を行います。バジェットがあれば優先され、なければ Deficit ティアを公平に分け合うという、たったこれだけのルールで動作しています。
