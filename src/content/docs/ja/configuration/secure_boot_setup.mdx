...
3. Microsoft および OEM ファームウェアの組み込みキーとともにキーを登録してください。
    ...
    :::caution[ASUS 製マザーボードで `--firmware-builtin` は使用しないでください]
...
CachyOS は [sbctl-batch-sign](https://github.com/CachyOS/CachyOS-Settings/blob/master/usr/bin/sbctl-batch-sign) を提供しています。これは `sudo sbctl verify` から署名が必要なファイルの一覧を取得し、一括で署名するスクリプトです。
