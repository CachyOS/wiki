...
:::caution
Btrfs スナップショットは通常のバックアップの代わりにはなりません。重要なデータはかならず適切にバックアップを取ってください。
:::
...
:::tip
CachyOS にはデフォルトで `snap-pac` がインストールされており、これにより Snapper は `pacman` を使ったパッケージのインストール、アップグレード、削除の前後に自動的にスナップショットを作成します。こうした操作の際に手動でスナップショットを作成する必要はありません。
:::
...
GUI で Btrfs、スナップショットなどを管理したい方には [Btrfs Assistant](https://gitlab.com/btrfs-assistant/btrfs-assistant) が初心者にとても優しいプロジェクトになっています。
...
<Steps>
  ...
  2. Snapper Settings タブに移動してください。
      ...
      **Snapshot Retention** の設定を以下のように変更してください。
      ...
      - 右上の `Save` ボタンと右下の `Apply systemd changes` をクリックする
      **snapper-cleanup.timer が有効になり、設定に基づいて古いスナップショットを自動的に削除するようになります。**
      :::caution
      ...
      ```
      `NUMBER_LIMIT` と `NUMBER_LIMIT_IMPORTANT` の値が `10` でない場合は更新してください。
      `CTRL + O` -> `Enter` -> `CTRL + X` でファイルを保存して閉じてください。
      :::
</Steps>
...
ここまでは Btrfs スナップショットとはなにかと、その使用例について説明しました。次に [openSUSE](https://www.opensuse.org/) の [Arvin Schnell](https://github.com/aschnell) が開発した [Snapper](http://snapper.io/) というツールを使った管理方法を説明します。
...
<details>
    <summary>利用可能なスナップショットの一覧と詳細を表示する</summary>
    ```bash
    sudo snapper list
    ```
    <details>
...
<details>
    <summary>新しいスナップショットを作成する</summary>
    ```bash
...
<details>
    ...
    <details>
        ...
        ```
        <details>
            <summary>例: スナップショット 729 の説明を変更する</summary>
            ```bash
...
:::
    <details>
    <summary>ひとつのスナップショットを削除する</summary>
    ```bash
    sudo snapper delete <snapshot_number>
    ```
    </details>
    <details>
    <summary>範囲を指定してスナップショットを削除する</summary>
    ```bash
