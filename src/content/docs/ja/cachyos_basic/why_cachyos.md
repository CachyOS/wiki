...
こうしたパッチはユーザーに提供される前に内部テストが行われ、安定性への影響がないことを確認しています。CachyOS が提供する全パッチの一覧については、[カーネル](/ja/features/kernel)を参照してください。

### 高度な CPU スケジューラーのサポート

CachyOS は最新の CPU スケジューラーの最適化を組み込んだカーネルを提供しており、高負荷時でもスムーズで応答性の高いデスクトップ体験を実現します。

* **EEVDF (デフォルトの Linux カーネルスケジューラー):** 実効スループットに優れており、CachyOS カーネルにはデスクトップの応答性を向上させるカスタムの[**EEVDF チューニング**](https://github.com/CachyOS/linux/blob/6.15/cachy/kernel/sched/fair.c#L79-81)が適用されています。

* **[BORE](https://github.com/firelzrd/bore-scheduler) (Burst-Oriented Response Enhancer):** 最大限の応答性を必要とするユーザー向けに、BORE スケジューラーをサポートしています。EEVDF を拡張するパッチセットで、高負荷な処理をしているときでもよりスムーズな体験を提供します。
...
CachyOS にはカスタムのハードウェア識別ツールが含まれており、システムに必要なドライバーとパッケージを自動的に識別してインストールします。自力でドライバーを探す必要がなくなるため、インストール後の手間と時間を節約できます。
...
-   **[CachyOS Package Installer](https://github.com/CachyOS/packageinstaller):** アプリケーションをかんたんにインストールできるグラフィカルユーザーインターフェース (GUI) です。
-   **[CachyOS Kernel Manager](https://github.com/CachyOS/kernel-manager):** リポジトリからカーネルをインストールしたり、独自のカーネルを設定したり、`sched-ext` フレームワークを管理することができるツールです。
...
-   **[systemd-boot-manager](https://github.com/CachyOS/systemd-boot-manager):** `systemd-boot` の新しいブートエントリーを自動生成するツールです。`/etc/sdboot-manage.conf` でかんたんに設定可能です。

## フレンドリーで活発なコミュニティ

CachyOS の最大の強みは、拡大し続けるコミュニティです。コミュニティメンバーは知識を共有し、助け合い、プロジェクトの発展に貢献しています。皆さまのフィードバックが CachyOS の継続的な改善につながります。

[**CachyOS Discord**](https://discord.gg/cachyos-862292009423470592)や[**CachyOS フォーラム**](https://discuss.cachyos.org/)からコミュニティに参加しましょう。
