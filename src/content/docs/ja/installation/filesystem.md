---
title: ファイルシステム
description: 利用可能なファイルシステムの説明と推奨事項 (ext4, f2fs, btrfs, xfs, zfs, bcachefs)
---
...
CachyOS ではスナップショット機能をすぐに使えるように、標準でサブボリューム構成を提供しています。
...
Bcachefs は Linux 向けの新しい高機能ファイルシステムで、信頼性と堅牢性、そして現代的なファイルシステムに求められる機能を網羅することを重視しています。

:::caution[注意]
Bcachefs はまだ実験的であり、問題が発生する可能性があります。
:::

### メリット

- BTRFS や ZFS と同様のコピーオンライト (CoW)
- 圧縮
- キャッシュ、データ配置
- レプリケーション
- スケーラブル
...
`bcachefs-dkms` がツリー外カーネルモジュールのサポートを提供します。
ユーザー空間ツールとして `bcachefs-tools` が必要です。
