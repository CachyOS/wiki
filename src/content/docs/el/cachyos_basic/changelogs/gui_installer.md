...
* **Πρόγραμμα εγκατάστασης:**
  * Το Shelly αντικατέστησε πλέον το Octopi ως διαχειριστής πακέτων GUI.
  * Ένα «καθαρό» στιγμιότυπο δημιουργείται πλέον αμέσως μετά την εγκατάσταση και διατηρείται μόνιμα, παρέχοντας ένα βασικό σημείο επαναφοράς.
  * Το `os-prober` του GRUB είναι πλέον ενεργοποιημένο από προεπιλογή.
  * Προσθήκη του MangoWM ως επιλογής επιφάνειας εργασίας με τα αντίστοιχα dotfile.
  * Αφαίρεση της επιφάνειας εργασίας UKUI.
  * Οι AMD GPU χρησιμοποιούν πλέον ένα διαφορετικό θέμα Plymouth, καθώς ο οδηγός amdgpu αδυνατεί να αποδώσει το προηγούμενο θέμα αξιόπιστα σε φορητούς υπολογιστές με συνδεδεμένη δευτερεύουσα οθόνη.
  * Εκκαθάριση και εκσυγχρονισμός της επιλογής πακέτων για το GNOME.
  * Προσθήκη επιλογής για την εγκατάσταση του `MangoWM` με το κέλυφος `DMS`.
* **CachyOS-Welcome:**
  * Προσθήκη υποστήριξης του DNS-over-HTTPS (DoH) μέσω του `blocky`.
  * Προσθήκη υποστήριξης για προσαρμοσμένους διακομιστές DNS και αυτόματη ένδειξη/επαναφορά DHCP.
  * Οι διακομιστές DNS εμφανίζουν πλέον μεταδεδομένα (περιοχή, αρχική σελίδα, φιλτράρισμα) και υποστηρίζουν μεμονωμένες δοκιμές καθυστέρησης.
  * Προσθήκη του διακόπτη διαχείρισης VRAM `dmemcg-booster` (εγκαθιστά επιπλέον το `plasma-foreground-booster` στο KDE).
  * Προσθήκη πλήρους υποστήριξης για πλοήγηση με το πληκτρολόγιο για λόγους προσβασιμότητας.
  * Αντικατάσταση των εικονιδίων PNG για τα μέσα κοινωνικής δικτύωσης με ευκρινή SVG, συμβατά με οθόνες HiDPI.
  * Προσθήκη του `wezterm` στον βοηθό τερματικού.
* **chwd:**
  * Προσθήκη εγγενούς εντοπισμού συσκευών USB (μέσω libusb/sysfs) και ανίχνευση τύπου πλαισίου.
  * Προσθήκη υποστήριξης για ενσωμάτωση του δακτυλικού αποτυπώματος (`fprint`) στο sudo.
  * Προσθήκη εντοπισμού οικογένειας/μοντέλου CPU για την υποστήριξη του `intel-lpmd`.
  * **Φορητές κονσόλες:** Προσθήκη ακριβών μοτίβων για το Xbox ROG Ally.
  * **Δίκτυο:** Προσθήκη του προφίλ Wi-Fi Marvell AVASTAR 88W8897 (Surface Pro 4).
  * Διαχωρισμός των προφίλ NVIDIA για φορητούς υπολογιστές και περιβάλλοντα επιφάνειας εργασίας.
  * Διαχωρισμός και ενημέρωση των προφίλ για εικονικές μηχανές.
* **cachyos-settings:** Αλλαγή του προεπιλεγμένου χρονοπρογραμματιστή I/O για NVMe από `none` σε `kyber`.

**Διορθώσεις:**

* **Πρόγραμμα εγκατάστασης:**
  * Η μέθοδος διαμέρισης εκτυπώνεται πλέον στο αρχείο καταγραφής εντοπισμού σφαλμάτων.
  * Τα παλιά πακέτα μικροκώδικα καταργούνται πλέον κατά την επαναχρησιμοποίηση ενός υπάρχοντος διαμερίσματος εκκίνησης.
* **CachyOS-Welcome:**
  * Διορθώθηκαν οι έλεγχοι συνδεσιμότητας που επέστρεφαν λανθασμένα θετικό αποτέλεσμα όταν το ping αποτύγχανε.
  * Διασφαλίστηκε ότι τα εικονίδια εξωτερικών συνδέσμων είναι ορατά σε σκουρόχρωμα θέματα.
  * Εμποδίστηκε η ταυτόχρονη εκτέλεση πολλαπλών παραθύρων της εφαρμογής καλωσορίσματος.
  * Προστέθηκε το `StartupWMClass` για βελτιωμένη αντιστοίχιση παραθύρων αρχείων `.desktop`.
* **chwd:**
  * Αφαιρέθηκε το hook `kms` από το `mkinitcpio.conf` σε μη φορητούς υπολογιστές για τη διόρθωση διενέξεων με τον οδηγό NVIDIA.
  * Η αναζήτηση εγκατεστημένων πυρήνων στα προφίλ NVIDIA έγινε πιο ακριβής.
  * Αφαιρέθηκε η εξαναγκασμένη συνεδρία Xorg από τα προφίλ NVIDIA 470xx (διορθώνει τη συμβατότητα με το `plasma-login-manager`).
  * Αφαιρέθηκε η παρωχημένη ρύθμιση `WaylandEnable=false` για το GDM στα προφίλ εικονικών μηχανών.
  * Διορθώθηκαν ψευδώς θετικές ανιχνεύσεις φορητών κονσολών (π.χ. ορισμένοι φορητοί υπολογιστές MSI εκλαμβάνονταν εσφαλμένα ως συσκευές MSI Claw).
* **cachyos-settings:**
  * Αφαιρέθηκε η διαχείριση ενέργειας `S01x` λόγω προβλημάτων με τον οδηγό NVIDIA 595.
  * Απενεργοποιήθηκε το `AggressiveVblank` λόγω προβλημάτων σχετικών με το VR στον οδηγό NVIDIA.
...
* **Πρόγραμμα εγκατάστασης:**
  ...
  * Ταξινόμηση της λίστας περιβαλλόντων επιφάνειας εργασίας, από τα πιο εύκολα και προσβάσιμα στα πιο σύνθετα, όπως οι διαχειριστές παραθύρων.
* **CachyOS-Welcome:**
...
* **chwd:**
  * Γενικά προφίλ για φορητές κονσόλες και βελτιωμένη υποστήριξη για τις κάρτες γραφικών τους.
  * Το fwupd είναι πλέον ενεργοποιημένες για τις φορητές κονσόλες της Lenovo.
...
* **Πρόγραμμα εγκατάστασης:**
  ...
  * Αφαίρεση εξαρτήσεων του xorg από τα περιβάλλοντα επιφάνειας εργασίας που βασίζονται στο Wayland.
* **ISO:**
...
* **Πρόγραμμα εγκατάστασης:**
  ...
  * Διόρθωση ζητήματος όπου η επιλογή ενός περιβάλλοντος επιφάνειας εργασίας, η επιστροφή σε προηγούμενο βήμα και μετά, η αλλαγή της επιλογής περιβάλλοντος επιφάνειας εργασίας είχε ως αποτέλεσμα να παραμένουν επιλεγμένα και τα δύο.
...
* **Επιφάνεια εργασίας:** Προσθήκη του **Niri** ως επιλογή περιβάλλοντος επιφάνειας εργασίας, συμπεριλαμβανομένων ορισμένων προρυθμισμένων dotfile.
...
- **Secure Boot:** Βελτίωση της δέσμης ενεργειών «sbctl-batch-sign» ώστε να υπογράφει μόνο τα επιθυμητά αρχεία.
...
- **Ρυθμίσεις:** Προσθήκη υποστήριξης για το DLSS Swapper. Πρόκειται για μια δέσμη ενεργειών που ενημερώνει αυτόματα και χρησιμοποιεί την πιο πρόσφατη έκδοση και τις προεπιλογές του DLSS.
...
- **cachyos-settings:** Προσθήκη δέσμης ενεργειών για τη διευκόλυνση της εκτέλεσης εφαρμογών μέσω Zink.
...
- Ενημέρωση των πακέτων που σχετίζονται με φορητές κονσόλες.
...
- **Επιφάνεια εργασίας:** Προσθήκη του περιβάλλοντος επιφάνειας εργασίας Cosmic στις επιλογές εγκατάστασης.
...
- **SecureBoot:** Προσθήκη δέσμης ενεργειών και οδηγού στο wiki για εύκολη υποστήριξη του Secure Boot.
...
- **Steam/Proton:** Διατίθεται πλέον η δέσμη ενεργειών «game-performance», η οποία μπορεί να προστεθεί στις επιλογές εκκίνησης του Steam.
...
- **Netinstall:** Το Cutefish αφαιρέθηκε από τις επιλογές εγκατάστασης περιβάλλοντος επιφάνειας εργασίας.
...
- **Εκκίνηση προγράμματος εγκατάστασης:** Εγκατάσταση του πιο πρόσφατου `archlinux-keyring` πριν από την έναρξη της εγκατάστασης για την αποφυγή προβλημάτων κατά τη λήψη της κλειδοθήκης μέσα στο chroot.
...
- **chwd:** Προσθήκη εντοπισμού υλικού για φορητές κονσόλες.
...
- **ISO:** Προσθήκη του `cachy-chroot`. Πρόκειται για μια δέσμη ενεργειών που βοηθά τον χρήστη να εισέλθει στο σύστημα μέσω chroot.
...
- **CachyOS-Deckify:** Παροχή παραλλαγής για φορητές κονσόλες (πειραματική), δείτε [εδώ](https://discuss.cachyos.org/t/information-experimental-cachyos-deckify/203) για περισσότερες λεπτομέρειες.
...
- **Netinstall:** Χρήση του lightdm αντί για το ly σε διάφορα περιβάλλοντα επιφάνειας εργασίας, λόγω σφάλματος στο ly.
...
- **Περιβάλλον επιφάνειας εργασίας:** Προσθήκη του περιβάλλοντος επιφάνειας εργασίας Qtile.
- **Ανασχεδιασμός του mhwd:** Επανεγγραφή σε Rust, απλοποιημένα προφίλ για κάρτες γραφικών και δικτύου και αφαίρεση παρωχημένου κώδικα.
...
- **rEFInd:** Χρήση του linux-cachyos-lts ως προεπιλογής. Η τρέχουσα έκδοση 6.2 φαίνεται να μην λειτουργεί σωστά με το rEFInd.
...
- Προσθήκη των περιβαλλόντων επιφάνειας εργασίας Budgie, MATE και LXDE στο netinstall.
...
- Προσθήκη του περιβάλλοντος επιφάνειας εργασίας UKUI στις επιλογές του netinstall.
- Προσθήκη του περιβάλλοντος επιφάνειας εργασίας Cinnamon στις επιλογές του netinstall.
...
- Προσθήκη δέσμης ενεργειών αυτόματης έκδοσης κατά τη δημιουργία του ISO.
...
- Αφαιρέθηκε το περιβάλλον επιφάνειας εργασίας Kofuku.
...
- Το copytoram απενεργοποιήθηκε εντελώς, καθώς προκαλεί πρόβλημα στην εγκατάσταση εκτός σύνδεσης.
...
- Εφαρμογή επιλεγμένης βελτιστοποίησης πυρήνα (θα δείτε μια λίστα με τις διάφορες αρχιτεκτονικές CPU για χειροκίνητη επιλογή)
